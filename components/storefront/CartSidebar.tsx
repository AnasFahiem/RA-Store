                        ...
                        <div className="p-4 flex items-center justify-between border-b border-border/50">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold font-heading uppercase text-foreground">
                                    Your Cart
                                    {/* DEBUG */}
                                    {/* <span className="ml-2 text-[10px] bg-red-500/10 text-red-500 rounded px-1">{useCart().isLoading ? 'Syncing...' : 'Ready'}</span> */}
                                </h2>
                            </div>
                                                                ...
                                                                <div key={`${item.productId}-${item.variant}`} className="flex gap-3 relative">
                                                                    ...
                                                                    <div className="flex-1 flex flex-col justify-between py-1">
                                                                        ...
                                                                        <div className="flex items-center justify-end">
                                                                            {/* Allow removing individual items from bundle? Usually simpler to remove whole bundle or allow quantity adjust. 
                                                                                 For sidebar simplicity and UX, let's keep basic adjust controls but they might break bundle if quantity < min req. 
                                                                                 Safest for MVP: Allow adjust, if logic breaks, server handles or we accept partial bundle. 
                                                                                 Or easier: Just show quantity for now, or allow remove. */}
                                                                            <div className="flex items-center gap-2">
                                                                                <button
                            ...
                            <div className="p-6 border-t border-border/50 bg-background/50 backdrop-blur-sm space-y-4">
                                <div className="flex justify-between items-center text-xl font-bold text-foreground">
                                    ...
                                    <span>
                                        {formatCurrency(
                                            (() => {
                                                // Identify bundles and calculate total
                                                const { groupedItems, standaloneItems } = items.reduce((acc, item) => {
                                                    if (item.bundleId) {
                                                        if (!acc.groupedItems[item.bundleId]) {
                                                            acc.groupedItems[item.bundleId] = { items: [], details: item.bundleDetails };
                                                ...
                                                return standaloneItems.reduce((sum, i) => sum + (i.price || 0) * i.quantity, 0) +
